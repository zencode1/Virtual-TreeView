...
type
  ...
  published
    ...
    property Margin: TDimension read FMargin write SetMargin{$IFDEF VT_VCL} default 4{$ENDIF};
    property MaxWidth: TDimension read FMaxWidth write SetMaxWidth{$IFDEF VT_VCL} default 10000{$ENDIF};
    ...
    property Spacing: TDimension read FSpacing write SetSpacing{$IFDEF VT_VCL} default cDefaultColumnSpacing{$ENDIF};
    property Style: TVirtualTreeColumnStyle read FStyle write SetStyle default vsText;
    property Tag: NativeInt read FTag write FTag default 0;
    property Text: string read GetText write SetText;
    property Width: TDimension read FWidth write SetWidth{$IFDEF VT_VCL} default 50{$ENDIF};
  end;
  ...
  protected
    ...
    procedure DrawButtonText(ACanvas: TCanvas; Caption: string; Bounds: TRect; Enabled, Hot: Boolean; DrawFormat: Cardinal;
      WrapCaption: Boolean);
    ...
    function HandleClick(P: TPoint; Button: TMouseButton;{$IFDEF VT_FMX}Shift: TShiftState;{$ENDIF} Force, DblClick: Boolean): Boolean; virtual;
...
{$ENDIF}
  ...
  published
    ...
    property DefaultHeight        : TDimension read FDefaultHeight write SetDefaultHeight{$IFDEF VT_VCL} default 19{$ENDIF};
    property Font                 : TFont read FFont write SetFont stored IsFontStored;
    property FixedAreaConstraints : TVTFixedAreaConstraints read FFixedAreaConstraints write FFixedAreaConstraints;
    property Height               : TDimension read FHeight write SetHeight{$IFDEF VT_VCL} default 19{$ENDIF};
    property Images               : TCustomImageList read FImages write SetImages;
    property MainColumn           : TColumnIndex read GetMainColumn write SetMainColumn default 0;
    property MaxHeight            : TDimension read FMaxHeight write SetMaxHeight{$IFDEF VT_VCL} default 10000{$ENDIF};
    property MinHeight            : TDimension read FMinHeight write SetMinHeight{$IFDEF VT_VCL} default 10{$ENDIF};
    ...
    property SplitterHitTolerance : TDimension read fSplitterHitTolerance write fSplitterHitTolerance{$IFDEF VT_VCL} default 8{$ENDIF};
...
uses
  ...
  , VirtualTrees.BaseAncestorFMX{to eliminate H2443 about inline expanding}
  ;
{$ELSE}
uses
  WinApi.ShlObj,
...
begin
  ...
  FMaxHeight := 10000;
  FFont := TFont.Create;
  FFont.{$IFDEF VT_FMX}OnChanged{$ELSE}OnChange{$ENDIF} := FontChanged;
  FParentFont := True;
...
{$ENDIF}
    ...
    begin
{$IFDEF VT_FMX}
      FImageChangeLink.Images:= FImages;
{$ELSE}
...
begin
  ...
  if not ParentFont then
    Font.{$IFDEF VT_FMX}Size{$ELSE}Height{$ENDIF} := MulDiv(Font.{$IFDEF VT_FMX}Size{$ELSE}Height{$ENDIF}, M, D);
end;
...
{$ENDIF}
    ...
    begin
      if DoColumnWidthTracking(FColumns.TrackIndex, {$IFDEF VT_FMX}TShiftState(Word(Message.Keys)){$ELSE}GetShiftState{$ENDIF}, FTrackPoint, P) then
      begin
        if Tree.UseRightToLeftAlignment then
        begin
    ...
    begin
      ...
      begin
        ...
        begin
          if ((Abs(FDragStart.X - P.X) > {$IFDEF VT_FMX}3{$ELSE}Mouse.DragThreshold{$ENDIF}) or (Abs(FDragStart.Y - P.Y) > {$IFDEF VT_FMX}3{$ELSE}Mouse.DragThreshold{$ENDIF})) then
          begin
{$IFDEF VT_VCL}
            Tree.StopTimer(HeaderTimer);
{$ENDIF}
  ...
  case Message.Msg of
    ...
    WM_NCMBUTTONDOWN:
      begin
        ...
        if InHeader(P) then
          TBaseVirtualTreeCracker(FOwner).DoHeaderMouseDown(TMOuseButton.mbMiddle, {$IFDEF VT_FMX}TShiftState(Word(TWMMouse(Message).Keys)){$ELSE}GetShiftState{$ENDIF}, P.X, P.Y + FHeight);
      end;
    WM_NCMBUTTONUP:
      begin
        ...
        begin
          ...
          TBaseVirtualTreeCracker(FOwner).DoHeaderMouseUp(TmouseButton.mbMiddle, {$IFDEF VT_FMX}TShiftState(Word(TWMMouse(Message).Keys)){$ELSE}GetShiftState{$ENDIF}, P.X, P.Y + Self.FHeight);
          TVirtualTreeColumnsCracker(FColumns).DownIndex := NoColumn;
          TVirtualTreeColumnsCracker(FColumns).CheckBoxHit := False;
        end;
    ...
    WM_LBUTTONDBLCLK, WM_NCLBUTTONDBLCLK, WM_NCMBUTTONDBLCLK, WM_NCRBUTTONDBLCLK :
      begin
        ...
        begin
            ...
            if DoColumnWidthDblClickResize(FColumns.TrackIndex, P, {$IFDEF VT_FMX}TShiftState(Word(TWMMouse(Message).Keys)){$ELSE}GetShiftState{$ENDIF}) then
        ...
        begin
          ...
          if Button = TMouseButton.mbLeft then
            TVirtualTreeColumnsCracker(FColumns).AdjustDownColumn(P);
              TVirtualTreeColumnsCracker(FColumns).HandleClick(P, Button, {$IFDEF VT_FMX}TShiftState(Word(TWMMouse(Message).Keys)),{$ENDIF} True, True);
        end;
      end;
...
{$ENDIF}
        ...
        begin
          ...
          begin
            if not (csDesigning in Tree.ComponentState) then
              DoBeforeColumnWidthTracking(FColumns.TrackIndex, {$IFDEF VT_FMX}TShiftState(Word(TWMMouse(Message).Keys)){$ELSE}GetShiftState{$ENDIF});
...
{$ENDIF}
        ...
        if not (csDesigning in Tree.ComponentState) and IsInHeader then
          TBaseVirtualTreeCracker(FOwner).DoHeaderMouseDown(TMouseButton.mbLeft, {$IFDEF VT_FMX}TShiftState(Word(TWMMouse(Message).Keys)){$ELSE}GetShiftState{$ENDIF}, P.X, P.Y + FHeight);
      end;
    WM_NCRBUTTONDOWN:
      begin
        with TWMNCRButtonDown(Message) do
          P := Tree.ScreenToClient(Point(XCursor, YCursor));
        if InHeader(P) then
          TBaseVirtualTreeCracker(FOwner).DoHeaderMouseDown(TMouseButton.mbRight, {$IFDEF VT_FMX}TShiftState(Word(TWMMouse(Message).Keys)){$ELSE}GetShiftState{$ENDIF}, P.X, P.Y + FHeight);
      end;
    WM_NCRBUTTONUP:
      if not (csDesigning in FOwner.ComponentState) then
        ...
        begin
          ...
          if InHeader(P) then
          begin
            HandleMessage := TVirtualTreeColumnsCracker(FColumns).HandleClick(P, TMouseButton.mbRight, {$IFDEF VT_FMX}TShiftState(Word(TWMMouse(Message).Keys)),{$ENDIF} True, False);
            TBaseVirtualTreeCracker(FOwner).DoHeaderMouseUp(TMouseButton.mbRight, {$IFDEF VT_FMX}TShiftState(Word(TWMMouse(Message).Keys)){$ELSE}GetShiftState{$ENDIF}, P.X, P.Y + FHeight);
          end;
          fWasDoubleClick := False;
        end;
...
{$ENDIF}
        ...
        case Message.Msg of
          WM_LBUTTONUP:
            with TWMLButtonUp(Message) do
            begin
              if TVirtualTreeColumnsCracker(FColumns).DownIndex > NoColumn then
                TVirtualTreeColumnsCracker(FColumns).HandleClick(Point(XPos, YPos), TMouseButton.mbLeft, {$IFDEF VT_FMX}TShiftState(Word(TWMMouse(Message).Keys)), {$ENDIF} False, False);
              if FStates <> [] then
                TBaseVirtualTreeCracker(FOwner).DoHeaderMouseUp(TMouseButton.mbLeft, {$IFDEF VT_FMX}TShiftState(Word(TWMMouse(Message).Keys)){$ELSE}KeysToShiftState(Keys){$ENDIF}, XPos, YPos);
              fWasDoubleClick := False;
            end;
          WM_NCLBUTTONUP:
            begin
              ...
              if not fWasDoubleClick and (FColumns.DownIndex > NoColumn) then
                TVirtualTreeColumnsCracker(FColumns).HandleClick(P, TMouseButton.mbLeft, {$IFDEF VT_FMX}TShiftState(Word(TWMMouse(Message).Keys)), {$ENDIF}True, False);
              TBaseVirtualTreeCracker(FOwner).DoHeaderMouseUp(TMouseButton.mbLeft, {$IFDEF VT_FMX}TShiftState(Word(TWMMouse(Message).Keys)){$ELSE}GetShiftState{$ENDIF}, P.X, P.Y + FHeight);
              Result := True;
              fWasDoubleClick := False;
            end;
        end;
        ...
        if TVirtualTreeColumnsCracker(FColumns).DownIndex > NoColumn then
        begin
          Invalidate(Columns[Columns.DownIndex]);
          TVirtualTreeColumnsCracker(FColumns).DownIndex := NoColumn;
        end;
    ...
    WM_MOUSEMOVE, WM_NCMOUSEMOVE{,WM_SETCURSOR}: // mouse capture and general message redirection
      begin
        ...
        if Message.Msg<>{WM_SETCURSOR}WM_MOUSEMOVE then
          begin
            P := Tree.ScreenToClient(Point(TWMMouseMove(pomMsg).XPos, TWMMouseMove(pomMsg).YPos));
      ...
      begin
        ...
        begin
          ...
          if Assigned(cursorService) then  }
            NewCursor:= Tree.Cursor;// cursorService.GetCursor;
          {$ELSE}
          ...
          if IsVSplitterHit and ((hoHeightResize in FOptions) or (csDesigning in Tree.ComponentState)) then
            NewCursor := {$IFDEF VT_FMX}crSizeNS{$ELSE}Screen.Cursors[crVSplit]{$ENDIF}
          else
            if IsHSplitterHit then
              NewCursor := {$IFDEF VT_FMX}crSizeWE{$ELSE}Screen.Cursors[crHSplit]{$ENDIF};
...
{$IFDEF VT_VCL}
      PixelFormat := pf32Bit;
{$ENDIF}
    SetSize({$IFDEF VT_FMX}Round{$ENDIF}(DragColumn.Width), {$IFDEF VT_FMX}Round{$ENDIF}(FHeight)); //TODO: round is not good here!
...
//TODO: to investigate - here client coordinates and non client coordinates have problem in FMX
...
{$ENDIF}
      ...
      {$IFDEF VT_FMX}Family{$ELSE}Name{$ENDIF} := UTF8ToString(S);
{$IFDEF VT_VCL}
        ReadBuffer(Dummy, SizeOf(Dummy));
...
{$IFDEF VT_FMX}
      Difference := Trunc(ToGo / ColCount);
      Rest := Frac(ToGo / ColCount);
{$ELSE}
...
{$ENDIF}
      ...
      WriteBuffer(DummyDimension, SizeOf(DummyDimension));
      Tmp := UTF8Encode({$IFDEF VT_FMX}Family{$ELSE}Name{$ENDIF});
      Dummy := Length(Tmp);
      WriteBuffer(Dummy, SizeOf(Dummy));
...
begin
  ...
  {$IFDEF VT_FMX}PaintInfo.TargetCanvas.Font.Assign(Header.Font){$ELSE}PaintInfo.TargetCanvas.Font := Header.Font{$ENDIF};
    ...
    if PaintInfo.ShowHeaderGlyph then
      ...
      begin
        if Assigned(Images) then
          HeaderGlyphSize := {$IFDEF VT_FMX}Point(16, 16){$ELSE}Point(Images.Width, Images.Height){$ENDIF}; //TODO 16px image
      end
      else
        with Self.TreeViewControl do
        begin
          if Assigned(CheckImages) then
            HeaderGlyphSize := {$IFDEF VT_FMX}Point(16, 16){$ELSE}Point(CheckImages.Width, CheckImages.Height){$ENDIF}; //TODO 16px image
        end
    else
...
{$ENDIF}
      begin
        PaintInfo.SortGlyphSize.cx := {$IFDEF VT_FMX}16{$ELSE}Self.TreeViewControl.ScaledPixels(16){$ENDIF};//TODO: 16px Image!
        PaintInfo.SortGlyphSize.cy := {$IFDEF VT_FMX}16{$ELSE}Self.TreeViewControl.ScaledPixels(4){$ENDIF};//TODO: 16px Image!
      end;
  ...
  begin
    ...
    begin
      ...
      if FCaptionText = '' then
        FCaptionText := WrapString(PaintInfo.TargetCanvas, FText, R, {$IFDEF VT_FMX}false{$ELSE}DT_RTLREADING and DrawFormat <> 0{$ENDIF}, DrawFormat);

      GetStringDrawRect(PaintInfo.TargetCanvas, FCaptionText, R, DrawFormat);
...
begin
  ...
  begin
    Columns := GetOwner as TVirtualTreeColumns;
    if Assigned(Columns) and (FColor <> TreeViewControl.{$IFDEF VT_FMX}Fill.{$ENDIF}Color) then
    begin
      FColor := TreeViewControl.{$IFDEF VT_FMX}Fill.{$ENDIF}Color;
      Changed(False);
    end;
  end;
end;
...
procedure TVirtualTreeColumns.DrawButtonText(ACanvas: TCanvas; Caption: string; Bounds: TRect; Enabled, Hot: Boolean;
...
begin
  ...
  begin
    ...
    GetTextExtentPoint32W(ACanvas{$IFDEF VT_VCL}.Handle{$ENDIF}, {$IFDEF VT_VCL}PWideChar{$ENDIF}(Caption), Length(Caption), Size);
    TextSpace := Bounds.Right - Bounds.Left;
    if TextSpace < Size.cx then
      Caption := ShortenString(ACanvas, Caption, TextSpace);
  end;
{$IFDEF VT_VCL}
  SetBkMode(ACanvas.Handle, TRANSPARENT);
{$ENDIF}
  if not Enabled then
    if Header.Tree.VclStyleEnabled then
    begin
{$IFDEF VT_FMX}
...
{$IFDEF VT_FMX}
    ACanvas.Fill.Color:= clBtnHighlight;
    DrawTextW(ACanvas, Caption, Length(Caption), Bounds, DrawFormat);
{$ELSE}
    SetTextColor(ACanvas.Handle, ColorToRGB(clBtnHighlight));
    Winapi.Windows.DrawTextW(ACanvas.Handle, PWideChar(Caption), Length(Caption), Bounds, DrawFormat);
{$ENDIF}
    OffsetRect(Bounds, -1, -1);
{$IFDEF VT_FMX}
    ACanvas.Fill.Color:= clBtnShadow;
    DrawTextW(ACanvas, Caption, Length(Caption), Bounds, DrawFormat);
{$ELSE}
...
function TVirtualTreeColumns.HandleClick(P: TPoint; Button: TMouseButton; {$IFDEF VT_FMX}Shift: TShiftState;{$ENDIF} Force, DblClick: Boolean): Boolean;
...
var
  OldWidth: TDimension;
  {$IFDEF VT_FMX}Canvas: TCanvas{$ELSE}DC: HDC{$ENDIF};
  I, Steps    : Integer;
  DX: TDimension;
  HeaderScrollRect,
    ScrollRect,
  ...
  NewBrush, LastBrush: {$IFDEF VT_FMX}TBrush{$ELSE}HBRUSH{$ENDIF};
...
begin
  ...
  begin
    ...
    begin
{$IFDEF VT_FMX}
      Canvas:= FHeader.Treeview.Canvas;
{$ELSE}
      ...
      with TreeViewControl do
        ...
        // by deleting a small stripe
          ...
          begin
            R := ScrollRect;
{$IFDEF VT_FMX}
          NewBrush := TBrush.Create(TBrushKind.Solid, Fill.Color);
          LastBrush := TBrush.Create(TBrushKind.Solid, Fill.Color);
          LastBrush.Assign(Canvas.Fill);
{$ELSE}
...
{$ENDIF}
            ...
            R.Right := R.Left + DX;
{$IFDEF VT_FMX}
          Canvas.FillRect(R, 0, 0, [], 1.0, NewBrush);
          Canvas.Fill.Assign(LastBrush);
          FreeAndNil(NewBrush);
          FreeAndNil(LastBrush);
{$ELSE}
...
{$ENDIF}
  ...
  PaintHeader({$IFDEF VT_FMX}TargetCanvas{$ELSE}FHeaderBitmap.Canvas{$ENDIF},
...
{$IFDEF VT_FMX}
            TargetCanvas.Fill.Color := Header.Background;
            TargetCanvas.FillRect(PaintRectangle, 0, 0, [], 1.0);
{$ELSE}
            TargetCanvas.Brush.Color := Header.Background;
            TargetCanvas.FillRect(PaintRectangle);
{$ENDIF}
            if IsDownIndex then
              DrawEdge(TargetCanvas{$IFDEF VT_VCL}.Handle{$ENDIF}, PaintRectangle, PressedButtonStyle, PressedButtonFlags)
            else
              ...
              if (Header.Style = hsPlates) and IsHoverIndex and
                ...
                DrawEdge(TargetCanvas{$IFDEF VT_VCL}.Handle{$ENDIF}, PaintRectangle, RaisedButtonStyle,
                  RaisedButtonFlags or RightBorderFlag)
              else
                DrawEdge(TargetCanvas{$IFDEF VT_VCL}.Handle{$ENDIF}, PaintRectangle, NormalButtonStyle,
        ...
        FHasImage := False;
        if Assigned(Images) then
          ImageWidth := {$IFDEF VT_FMX}16{$ELSE}Images.Width{$ENDIF} //TODO: 16 px image!!!
        else
          ImageWidth := 0;
        ...
        begin
          ...
          begin
            ...
            Images.Draw(TargetCanvas, {$IFDEF VT_FMX}RectF(GlyphPos.X, GlyphPos.Y, GlyphPos.X+16, GlyphPos.Y+16){$ELSE}GlyphPos.X, GlyphPos.Y{$ENDIF}, FImageIndex{$IFDEF VT_VCL}, IsEnabled{$ENDIF}); //TODO: 16px Image!!!
          end
          else
          begin
            with Header.Tree do
          ...
          FImageRect.Right := FImageRect.Left + {$IFDEF VT_FMX}16{$ELSE}ColImageInfo.Images.Width{$ENDIF};//TODO: 16px Image!!!
          FImageRect.Bottom := FImageRect.Top + {$IFDEF VT_FMX}16{$ELSE}ColImageInfo.Images.Height{$ENDIF};//TODO: 16px Image!!!
        end;
...
{$ENDIF}
          begin
            //UtilityImages.Draw(TargetCanvas, {$IFDEF VT_FMX}RectF(SortGlyphPos.X, SortGlyphPos.Y, SortGlyphPos.X + 16, SortGlyphPos.Y + 16){$ELSE}SortGlyphPos.X, SortGlyphPos.Y{$ENDIF}, SortIndex);//TODO: 16px Image!!!
            PaintInfo.DrawSortArrow(Header.SortDirection);
          end;
        end;
...
*)
        ...
        begin
{$IFDEF VT_FMX}
          saveS:= TargetCanvas.SaveState;
          try
		  	    FHeader.Tree.DoAdvancedHeaderDraw(PaintInfo, ActualElements);
          finally
            TargetCanvas.RestoreState(saveS); //must i free it???
          end;
{$ELSE}
...
{$IFDEF VT_FMX}
      SaveS:= TargetCanvas.SaveState;
      try
        TargetCanvas.IntersectClipRect(RectF(Max(TargetRect.Left, Target.X), Target.Y + R.Top,
                                      Min(TargetRect.Right, MaxX), TargetRect.Bottom));
        PaintColumnHeader(Run, TargetRect);
      finally
        TargetCanvas.RestoreState(SaveS);
      end;
{$ELSE}
