...
{$IFDEF VT_FMX}
  ...
  TVTPaintContext = FMX.Graphics.TCanvas;
  TVTBrush = TBrush;
...
const
  ...
  DefaultMiscOptions = [{$IFDEF VT_VCL}toAcceptOLEDrop, {$ENDIF}toFullRepaintOnResize, toInitOnSave, toToggleOnDblClick, toWheelPanning, toEditOnClick];
...
type
  ...
  TCustomVirtualTreeOptionsHelper = class helper for TCustomVirtualTreeOptions
    function TreeView : TVTCracker;
  end;

  TScrollBarOptionsHelper = class helper for TScrollBarOptions
    function TreeView : TVTCracker;
  end;
...
{$IFDEF VT_FMX}
  if ImageInfo[iiNormal].Index > -1 then
    ImageInfo[iiNormal].YPos := CellRect.Top + VAlign - 16 / 2; //TODO: 16px Image!
  if ImageInfo[iiState].Index > -1 then
    ImageInfo[iiState].YPos := CellRect.Top + VAlign - 16 / 2; //TODO: 16px Image!
  if ImageInfo[iiCheck].Index > -1 then
    ImageInfo[iiCheck].YPos := CellRect.Top + VAlign - 16 / 2; //TODO: 16px Image!
{$ELSE}
  if ImageInfo[iiNormal].Index > -1 then
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if ((tsUseThemes in TreeStates) or ((toThemeAware in ToBeSet) {$IFDEF VT_VCL}and StyleServices.Enabled{$ENDIF})) and (toUseExplorerTheme in (ToBeSet + ToBeCleared)) and
...
{$ENDIF}
        ...
        begin
          ...
          begin
            if ((toThemeAware in ToBeSet) {$IFDEF VT_VCL}and StyleServices.Enabled{$ENDIF}) then
...
{ TCustomVirtualTreeOptionsHelper }

function TCustomVirtualTreeOptionsHelper.TreeView: TVTCracker;
begin
  Result := TVTCracker(FOwner);
end;

//----------------------------------------------------------------------------------------------------------------------

{ TScrollBarOptionsHelper }

function TScrollBarOptionsHelper.TreeView: TVTCracker;
begin
  Result := TVTCracker(FOwner);
end;
