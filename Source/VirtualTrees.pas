...
{$ELSE}
  ...
  , Winapi.Windows, Winapi.oleacc, Winapi.Messages, Winapi.ActiveX, Winapi.CommCtrl, Winapi.UxTheme, Winapi.ShlObj
  , Vcl.Graphics, Vcl.Controls, Vcl.Forms, Vcl.ImgList, Vcl.StdCtrls, Vcl.Menus, Vcl.Printers, Vcl.Themes
  , VirtualTrees.Types
  , VirtualTrees.Colors
...
type
  ...
  TCanvas                  = VirtualTrees.Types.TCanvas;
...
begin
  ...
  CF_HTML := {$IFDEF VT_FMX}0{$ELSE}RegisterVTClipboardFormat(CFSTR_HTML, TCustomVirtualStringTree, 80){$ENDIF};
  CF_VRTFNOOBJS := {$IFDEF VT_FMX}0{$ELSE}RegisterVTClipboardFormat(CFSTR_RTFNOOBJS, TCustomVirtualStringTree, 84){$ENDIF};
  CF_VRTF := {$IFDEF VT_FMX}0{$ELSE}RegisterVTClipboardFormat(CFSTR_RTF, TCustomVirtualStringTree, 85){$ENDIF};
  CF_CSV := {$IFDEF VT_FMX}0{$ELSE}RegisterVTClipboardFormat(CFSTR_CSV, TCustomVirtualStringTree, 90){$ENDIF};
...
begin
  ...
  begin
    ...
    if Enabled then // Otherwise only those colors are used, which are passed from Font to Canvas.Font.
       Canvas.{$IFDEF VT_FMX}Fill{$ELSE}Font{$ENDIF}.Color := Colors.NodeFontColor
    else
      Canvas.{$IFDEF VT_FMX}Fill{$ELSE}Font{$ENDIF}.Color := Colors.DisabledColor;

    if (toHotTrack in TreeOptions.PaintOptions) and (Node = HotNode) then
    begin
      {$IFDEF VT_VCL}if not (tsUseExplorerTheme in TreeStates) then{$ENDIF}
      begin
        Canvas.Font.Style := Canvas.Font.Style + [TFontStyle.fsUnderline];
        Canvas.{$IFDEF VT_FMX}Fill{$ELSE}Font{$ENDIF}.Color := Colors.HotColor;
      end;
    end;
      ...
      begin
        ...
        begin
          if (LastDropMode = dmOnNode) or (vsSelected in Node.States) or InternalIsCellSelected(Node, Column) then
            Canvas.{$IFDEF VT_FMX}Fill{$ELSE}Font{$ENDIF}.Color := Colors.GetSelectedNodeFontColor(True); // See #1083, since drop highlight color is chosen independent of the focus state, we need to choose Font color also independent of it.
        end
        else
          if (vsSelected in Node.States) or InternalIsCellSelected(Node, Column) then
          begin
            Canvas.{$IFDEF VT_FMX}Fill{$ELSE}Font{$ENDIF}.Color := Colors.GetSelectedNodeFontColor({$IFDEF VT_FMX}IsFocused{$ELSE}Focused{$ENDIF} or (toPopupMode in TreeOptions.PaintOptions));
          end;
      end;
    end;
  end;
...
{$IFDEF VT_FMX}
      Canvas.Fill.Color := Colors.DisabledColor;
{$ELSE}
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (vsSelected in Node.States) or InternalIsCellSelected(Node, Column) then
        begin
          if {$IFDEF VT_FMX}IsFocused{$ELSE}Focused{$ENDIF} or (toPopupMode in TreeOptions.PaintOptions) then
          Canvas.{$IFDEF VT_FMX}Fill{$ELSE}Font{$ENDIF}.Color := Colors.GetSelectedNodeFontColor({$IFDEF VT_FMX}IsFocused{$ELSE}Focused{$ENDIF} or (toPopupMode in TreeOptions.PaintOptions))
          else
            Canvas.{$IFDEF VT_FMX}Fill{$ELSE}Font{$ENDIF}.Color := Colors.NodeFontColor;
        end;
...
{$IFDEF VT_FMX}
    DrawTextW(Canvas, Text, Length(Text), R, DrawFormat);
    //Canvas.FillText(R, (Text), true, 1.0, [], TTextAlign.Leading);
{$ELSE}
    if Canvas.TextFlags and ETO_OPAQUE = 0 then
      SetBkMode(Canvas.Handle, TRANSPARENT)
    else
...
begin
  ...
  begin
    ...
    Inc(Result, DoTextMeasuring(Canvas, Node, Column, Text).{$IFDEF VT_FMX}Width{$ELSE}cx{$ENDIF});
...
begin
  ...
  if not Done then
    Result := ShortenString(Canvas, S, Width, EllipsisWidth);
end;
...
begin
  GetTextExtentPoint32W(Canvas{$IFDEF VT_VCL}.Handle{$ENDIF}, {$IFDEF VT_VCL}PWideChar{$ENDIF}(Text), Length(Text), Result);
  if vsMultiLine in Node.States then
  begin
...
{$ENDIF}
  ...
  if BidiMode <> bdLeftToRight then
    ...
    DrawFormat := DrawFormat or DT_LEFT;
{$IFDEF VT_FMX}
  DrawTextW(Canvas, S, Length(S), PaintInfo.CellRect, DrawFormat);
  //Canvas.FillText(PaintInfo.CellRect, S, true, 1.0, [], TTextAlign.Leading);
{$ELSE}
...
initialization
  {$IFDEF VT_VCL}TCustomStyleEngine.RegisterStyleHook(TVirtualStringTree, TVclStyleScrollBarsHook);{$ENDIF}

finalization
  {$IFDEF VT_VCL}TCustomStyleEngine.UnRegisterStyleHook(TVirtualStringTree, TVclStyleScrollBarsHook);{$ENDIF}																		  
